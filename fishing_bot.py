...
class PixelBladeFishingBot:
    ...
    def detect_multiplier_text(self, gray: np.ndarray) -> bool:
        """Detect '10x' text pattern (shows need to press E 10 times)"""
        ...
        for contour in contours:
            ...
            if 15 < w < 80 and 15 < h < 80:  # Reasonable text size
                # Extract region and check for '10x' character pattern
    ...
    def detect_10x_pattern(self, roi: np.ndarray) -> bool:
        ...
        # Check for '1' and '0' and 'x' characters
        # Simple heuristic: look for digit patterns and 'x' pattern
        has_vertical_lines = np.sum(roi[:, :w//3]) > 50  # '1' has vertical lines
        has_circular_shape = np.sum(roi[:, w//3:2*w//3]) > 50  # '0' is circular
        has_diagonal_lines = self.detect_x_pattern(roi[:, 2*w//3:])  # 'x' has diagonals
        
        return has_vertical_lines and has_circular_shape and has_diagonal_lines
    ...
    def fishing_cycle(self):
        ...
        # Wait for circle overlap or 10x text
        ...
        image = self.capture_screen()
        if self.detect_multiplier_text(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)):
            logger.info("Detected '10x' - spamming E 10+ times until UI disappears")
            self.spam_until_ui_gone()
        else:
